...
export interface INoteRepository {
  ...
  getNotesRange(startTimestamp: number, endTimestamp: number): Promise<Record<number, INote>>;
...
export const noteRepository: INoteRepository = {
  ...
  async getNotesRange(startTimestamp, endTimestamp) {
    ...
    const noteKeys = keys.filter((key: string) => key.startsWith(NOTE_KEY_PREFIX));
    const dateStrings = noteKeys.map((key: string) => key.replace(NOTE_KEY_PREFIX, ''));
  ...
  async getExistingNoteDates() {
    ...
    return keys
      ...
      .map((key: string) => {
        try {
          return convertNoteKeyDateToTimestamp(key.replace(NOTE_KEY_PREFIX, ''));
        } catch {
          return 0;
        }
      })
